...
import { View, StyleSheet, SectionList, Dimensions, FlatList } from 'react-native';
...
export const KitchenHomeScreen = () => {
  ...
  // État pour les notifications informatives (WebSocket)
  ...
  // Fonction pour afficher une notification informative
  ...
  useEffect(() => {
    const dimensionsListener = Dimensions.addEventListener('change', () => {
      // Force un re-rendu quand l'orientation change
      setRefreshing(true);
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right']}>
      ...
      <View style={styles.contentContainer}>
        {isTablet && isLandscape ? (
          // Layout optimisé pour tablette en mode paysage
          <View style={styles.tabletLandscapeLayout}>
            {/* Colonne des commandes à préparer */}
          ...
          // Layout standard (téléphone ou tablette en portrait)
          <SectionList
            sections={sections}
            keyExtractor={(item) => item.id.toString()}
      ...
      {/* Snackbar d'information (WebSocket) */}
      <Snackbar
...
const styles = StyleSheet.create({
  ...
  infoSnackbar: {
    backgroundColor: '#4CAF50', // Vert pour les notifications informatives
  },
